...
contract SchnorrSetVerifier is ISchnorrSetVerifier, Ownable2Step {
    ...
    /// @inheritdoc ISchnorrSetVerifier
    function isSigner(address signer) external view override returns (bool isSigner) {
        isSigner = signerIndexes[signer] != 0;
    }
    ...
    /// @inheritdoc ISchnorrSetVerifier
    function getSignerIndex(address signer) external view override returns (uint256 index) {
        index = signerIndexes[signer];
    }

    /// @inheritdoc ISchnorrSetVerifier
    function getTotalSigners() external view override returns (uint256 totalSigners) { 
        bytes memory pubKeys = SSTORE2.read(pointer);
